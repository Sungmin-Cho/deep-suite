...
이 가이드는 deep-suite의 통합 artifact 워크플로우에 참여하는 7개 플러그인이 호환 가능한 범용 AI 에이전트 도구에서 실제 프로젝트 중 **어떻게 함께 동작하는지** 설명한다. 개별 플러그인의 기능 목록이 아니라, 개발자 관점의 **통합 사용 흐름**에 초점을 맞춘다. (deep-goal 과 deep-loop 은 이 artifact 데이터 흐름의 참여자가 아니라 suite 의 goal-compiler·loop-orchestration 계층이므로 curated artifact 서브셋에서 제외하지만, 선택표는 스위트 전체 플러그인을 다룬다.)
