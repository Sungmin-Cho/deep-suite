...
Deep Suite는 기존 사용자 호환성을 위해 `claude-deep-suite` marketplace key를 유지하되, 플러그인 저장소명은 런타임 중립적인 `deep-*` 를 쓴다. protocol과 artifact 계약은 호스트 중립적이며, 설치와 호출은 adapter별로 달라진다.
...
| Grok | 문서화된 플러그인별 adapter | 지원되는 plugin/runtime 조합에서 slash 호출. 예: deep-loop의 attended macOS 실행 |
| Copilot CLI, Gemini CLI, Agent SDK 및 유사 호스트 | 설치된 `skills/<skill>/SKILL.md` | 해당 호스트의 skill/tool adapter를 통해 이식 가능한 skill 호출 |

아래 시나리오 예시는 짧게 읽히도록 대부분 slash-command 형식을 사용한다. 활성 호스트에 맞는 native alias 또는 portable skill 진입점을 적용한다. protocol 이식성은 모든 호스트의 hook, sandbox, unattended runtime 지원이 같다는 뜻이 아니며, 플러그인별 runtime 제약이 우선한다.
...
<!-- deep-suite:auto-generated:problem-plugin-table-guide-ko:start -->
...
<!-- deep-suite:auto-generated:problem-plugin-table-guide-ko:end -->

**호출 방식은 별도 관심사다** — 선택표는 특정 호스트를 가정하지 않고 문제를 라우팅한다. Claude Code는 주로 slash 문법, Codex는 `$plugin:skill`, Grok은 지원되는 플러그인별 slash adapter, 기타 호환 에이전트는 설치된 `skills/<skill>/SKILL.md` 진입점을 각자의 adapter로 소비한다.
...
deep-review는 capability에 따라 독립 reviewer 역할을 선택하고 활성 호스트에서 adapter별 dispatch를 수행한다. Claude Code는 named reviewer agent 또는 native bridge와 격리된 Codex 역할을 사용할 수 있고, Codex는 history-free native generic-subagent 역할 두 개와 감지된 추가 provider를 사용할 수 있다. 기타 AI 에이전트 도구는 실제로 제공하는 reviewer adapter와 provider만 사용하며, 범용 Claude bridge를 가정하지 않는다:

- **Stage 1** — git 상태 감지 (clean/staged/unstaged).
- **Stage 2** — 프로젝트 규칙(`.deep-review/rules.yaml`) 로드.
- **Stage 3** — Capability-aware routing으로 독립 reviewer 역할을 배정하고 가능한 provider family로 선택적 확장.
...
2. **deep-review는 PR 전에 실행하라** — capability 기반 독립 reviewer context가 self-review가 놓친 것을 잡는다. 실제 실행 가능한 native·cross-provider 역할은 활성 host adapter가 결정한다.
