...
deep-review assigns capability-selected independent reviewer roles over the full diff and uses adapter-specific dispatch for the active host. Claude Code can use a named reviewer agent or its native bridge plus isolated Codex roles; Codex supplies two history-free native generic-subagent roles and can add another detected provider. Other AI agent tools use only the reviewer adapters and providers they actually expose—there is no universal Claude bridge assumption:

- **Stage 1** — Detect git state (clean/staged/unstaged).
- **Stage 2** — Load project rules (`.deep-review/rules.yaml`).
- **Stage 3** — Capability-aware routing assigns independent reviewer roles and optionally expands across available provider families.
...
2. **Run deep-review before PRs** — capability-selected independent reviewer contexts catch what self-review misses. The active host adapter determines which native and cross-provider roles can actually run.
